...
class QuestionGenerator:
    def __init__(self):
        ...
        myText = self.canvas.create_text(100, 50, text = "问题区")
        self.canvas2 = Canvas(window, bg="white", width=self.width, height=375)
        self.canvas2.pack()
        myText = self.canvas2.create_text(100, 50, text = "答案区")
        # self.canvas3 = Canvas(window, bg="white", width=self.width, height=100)
        # self.canvas3.pack()
        # myText = self.canvas3.create_text(100, 30, text="提示区")
        #
        # text = Text(window, bg="white", width = 30,height=3)
        # text.insert(1.0, "请回答:")
        # text.insert(END, self.insertContent(self.num))

        # text.pack()

        frame = Frame(window)
        frame.pack()
        btGenerator = Button(frame, text = "出题", command = self.generatorQuestion)
